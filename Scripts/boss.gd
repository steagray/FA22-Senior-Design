...
func _process(delta):
	# Checks every frame if health is at a certain point
	# I make a percentage here so you can make the health what
		# you like and not have to recode here.
	current_health_percent == (health / current_health_percent)
	if current_health_percent == .75:
		# 75% HP action here
		pass
	if current_health_percent <= .5:
		# 50% HP action here
		pass
